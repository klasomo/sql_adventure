var Pageflip = (function(fn) {
    ...
    fn = function(mainElement, pages) {
      ...
      var _this = this;
      const mainEl = this._el(mainElement).element;
      if (!mainEl) return;
    ...
    fn.prototype.addClickOnGrabbers = function() {
      ...
      const pageLeftEl = this._el('.page-left').element;
      if (pageLeftEl) {
        pageLeftEl.classList.add('disable-click');
      }
    };
    ...
    fn.prototype.addPage = function(page, selector) {
      const el = this._el(selector).element;
      if (el) {
        if (this.pages[page] !== undefined) {
          el.style.backgroundImage = 'url("'+ this.pages[page] +'")';
        } else {
    ...
    fn.prototype.buildMarkup = function(mainElement) {
      const pageFlipEl = this._el('#page-flip').element;
      if (!pageFlipEl) return;
    ...
    fn.prototype._el = function(selector) {
      ...
      if (!element) {
        ...
        return { element: null, addClass: function() {}, removeClass: function() {} };
      }
      return {
        element: element,
  ...
  document.addEventListener('DOMContentLoaded', function() {
    var pageflip = new Pageflip('#yourElementSelector', ['page1.jpg', 'page2.jpg']);
  });
